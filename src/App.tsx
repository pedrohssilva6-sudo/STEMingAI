...
function App() {
  ...
  function drawerContent() {
    ...
    return <div className="drawer-section"><p>{inspectorText()}</p><button className="ghost" onClick={() => setSelectedId('live')}>Ler estado atual</button><button className="ghost" onClick={() => setConnectFrom(selectedId)}>{connectFrom ? `Conectando a partir de ${connectFrom}` : 'Criar conexao a partir deste item'}</button><h3>Ambiente</h3>{environment ? <p className="logic-row">{environment.type} · {environment.dimension} · {environment.coordinateSystem}</p> : <p className="issue warning">Sem EnvironmentSpec.</p>}<h3>Contrato do modelo</h3><p className="logic-row">{contract.domain} · {contract.fidelityLevel}</p><p>{contract.learningGoal}</p><h3>Estado lógico</h3><p className="logic-row">{Object.keys(sceneState.engine.shapes).length} formas · {Object.keys(sceneState.engine.relations).length} relações · {Object.keys(sceneState.engine.invariants).length} invariantes · {sceneState.engine.eventLog.length} eventos</p><h3>Validacao</h3>{sceneState.issues.length ? sceneState.issues.map((issue) => <p key={`${issue.path}-${issue.message}`} className={`issue ${issue.severity}`}>{issue.path}: {issue.message}</p>) : <p className="issue ok">SceneSpec v1 valida para execucao local.</p>}<h3>Limitacoes</h3>{scene.model_limitations.map((item) => <p key={item} className="limit">{item}</p>)}</div>;
  }
